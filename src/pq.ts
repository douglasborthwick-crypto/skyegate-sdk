...
// A content token is an attestation JWT, so its companion must be signed by the attestation
// post-quantum key. The JWKS also publishes a trust-profile key; a companion under it is refused.
  ...
  if (!mlDsaPromise) {
    mlDsaPromise = (async () => {
      try {
        ...
        // Turbopack and Vite to leave the import to the runtime, which resolves it from
        // node_modules like any other package.
...
export async function verifyPqCompanion(
  ...
  if (kid !== EXPECTED_PQ_KID) {
    return { status: 'refuted', kid, reason: `pqJwt is signed by "${kid}"; a content token's companion must be signed by "${EXPECTED_PQ_KID}"` };
  }
